...
> 以下是可直接使用的描述模板，**方括号内的量化数据请按你的实际观测值填写**。

**项目名称**：远行者 · AI 智能旅游助手

**技术栈**：Vue 3 / Vite / Pinia / Element Plus / ECharts · Node.js / Express / LangChain / MySQL 8 · SSE / Docker / Nginx

**项目描述**：
> 面向 C 端的 AI 旅游规划系统，用户输入目的地、预算、天数与偏好，系统调用大语言模型流式生成逐日行程，并在前端实现边生成边渲染的渐进式呈现。涵盖 AI 多轮对话、行程持久化、收藏管理与旅行足迹可视化。

**核心职责与亮点**（建议挑 4-5 条）：

1. **设计并实现 AI 网关层**，针对 LLM 调用慢、贵、易失败的特点，采用信号量控制并发、令牌桶按用户限流、总时长与单块停滞双重超时熔断、LRU 缓存复用结果，并通过健康检查接口暴露全量运行时指标；异常中断时在 `finally` 中关闭底层迭代器避免连接泄漏。

2. **基于 SSE 实现全链路流式传输**：后端采用 Fetch + `ReadableStream` 手写 SSE 协议解析（`EventSource` 不支持 POST 与自定义请求头、Axios 不支持读流式响应体），配合 Nginx 关闭代理缓冲与 gzip，解决中间层缓冲导致打字机效果失效的问题；客户端断开后立即停止读取模型流，避免无效 token 消耗。

3. **实现增量 JSON 解析器**，通过字符级状态机（区分字符串内括号与转义字符）从模型输出的未闭合 JSON 中实时提取完整片段，实现行程卡片「逐张浮现」的渐进式渲染，[将用户感知等待时间降低约 XX%]。

4. **构建模型输出容错与自愈机制**：三重 JSON 解析策略、缺失字段结构兜底、预算分项按比例缩放至严格等于用户预算，[将模型返回数据的可用性从 XX% 提升至 XX%]。

5. **设计分级鉴权与优雅降级**：按接口性质区分 `authRequired` 与 `authOptional`，兼顾数据安全与零门槛体验；数据库连接池惰性初始化，依赖故障时仅降级数据库功能，AI 核心链路不受影响。

6. **容器化部署**：编写前后端多阶段构建 Dockerfile（分离构建与运行阶段，非 root 用户运行，dumb-init 保障优雅停机），使用 docker-compose 编排 MySQL / 后端 / Nginx 三服务，通过健康检查驱动启动顺序。
